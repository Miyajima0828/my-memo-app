    ...
    <div id="result" class="relative d-flex justify-content-center ">
        ...
        <div class="absolute z-50 mr-2 w-full bg-white rounded-md shadow-lg" >
            <ul id="list1" class="z-51">
                ...
                <label>
                    <li class="list-none">
                        <p class="relative absolute top-0 pl-10 text-justify rounded-md hover:bg-sky-100 focus:bg-sky-100" tabindex="0" wire:keydown.enter="onClickUpdate('{{$post->sub}}')" wire:click="onClickUpdate('{{$post->sub}}')"><span class="inline-block w-1/3">{{Str::limit($post->main, 40)}}</span><span class="inline-block w-1/3">{{Str::limit($post->sub, 40)}}</span><span class="inline-block w-1/3">{{Str::limit($post->text, 40)}}</span></p>
                    </li>
                </label>
                @endforeach
            </ul>
